...
class Parser:
    ...
    def parse_query_webqsp(self, query, mid_list):
        ...
        if re_data.match(r'ORDER BY .*\?\w*.* LIMIT 1', lines[-1]):
            lines[-1] = lines[-1].replace('LIMIT 1', '').strip()
            lines.append('LIMIT 1')

        if re_data.match(r'LIMIT \d*', lines[-1]):  # TODO LIMIT n
    ...
    def normalize_body_lines(self, lines, filter_string_flag=False):
        ...
        if re_data.match(r'FILTER \(\?\w* (>|<|>=|<=) .*', lines[-2]) \
                or re_data.match(r'FILTER \(xsd:integer\(\?\w*\) (>|<|>=|<=) .*', lines[-2]):

            compare_line = lines.pop(-2)
            compare_var = re_data.findall(r'\?\w*', compare_line)[0]
            compare_operator = re_data.findall(r'(>|>=|<|<=)', compare_line)[0]
            operator_mapper = {'<': 'lt', '<=': 'le', '>': 'gt', ">=": "ge"}
            if "^^xsd:dateTime" in compare_line:  # dateTime
                compare_value = re_data.findall(r'".*"\^\^xsd:dateTime', compare_line)[0]
            else:  # number
        ...
        if lines[-1] == 'LIMIT 1':
            ...
            compare_var = re_data.findall(r'\?\w*', order_line)[0]
        ...
        if len(lines) >= abs(range_line_num) and lines[range_line_num].startswith('FILTER(NOT EXISTS {?'):
            ...
            range_start_time = re_data.findall(f'".*"\^\^', range_lines[2])[0].split("^^")[0].strip('"')
...
def augment_with_s_expr_webqsp(split, check_execute_accuracy=False):
    ...
    dataset = load_json(f'D:\桌面\毕业论文\ChatKBQA-main\ChatKBQA-main\data\CCKS\origin\CCKS_{split}_Sexper.txt')
    ...
    for i, data in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        if (i + 1) % 100 == 0:
            print(
                f'In the First {i + 1} questions, S-Expression Gen rate [{split}]: {hit_num}, {total_num}, {hit_num / total_num}, {i + 1}')
            if check_execute_accuracy:
                print(
                    f'In the First {i + 1} questions, Execute right rate [{split}]: {execute_hit_num}, {total_num}, {execute_hit_num / total_num}, {i + 1}', )

    print(f'S-Expression Gen rate [{split}]: {hit_num}, {total_num}, {hit_num / total_num}, {len(dataset)}')
    print(
        f'Execute right rate [{split}]: {execute_hit_num}, {total_num}, {execute_hit_num / total_num}, {len(dataset)}', )
