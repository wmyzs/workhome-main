...
def lisp_to_sparql(lisp_program: str):
        ...
        if subp[0] == 'JOIN':
            if isinstance(subp[1], list):  # R relation
                ...
                if subp[2][:2] in ["m.", "g."]:  # entity
                    ...
                    if re_data.match(r'[\w_]*\.[\w_]*\.[\w_]*', subp[2]):
                        # 2-hop relation
                        pass
                    else:
                        ...
                        if subp[2].__contains__('^^'): # literal with datatype
                            ...
                            if data_type not in ['integer', 'float', 'dateTime','date']:
                                ...
                                subp[2] = f'"{subp[2].split("^^")[0]}"^^<{subp[2].split("^^")[1]}>'
                        elif re_data.match("[a-zA-Z_]*\.[a-zA-Z_]*", subp[2]): # type e.g. education.university
                            subp[2]='ns:'+subp[2]
            ...
            if re_data.match(r'\d+', subp[2]) or re_data.match(r'"\d+"^^xsd:integer', subp[2]): # integer
    ...
    for clause in clauses:
        variables = re_data.findall(r"\?\w*", clause)
        if variables:
