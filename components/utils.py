...
illegal_xml_re = re_data.compile(u'[\x00-\x08\x0b-\x1f\x7f-\x84\x86-\x9f\ud800-\udfff\ufdd0-\ufddf\ufffe-\uffff]')
...
def extract_mentioned_relations_from_sparql(sparql:str):
    ...
    relation_tokens = []
    for t in toks:
        if (re_data.match("ns:[a-zA-Z_0-9]*\.[a-zA-Z_0-9]*\.[a-zA-Z_0-9]*", t.strip())
            or re_data.match("ns:[a-zA-Z_0-9]*\.[a-zA-Z_0-9]*\.[a-zA-Z_0-9]*\.[a-zA-Z_0-9]*", t.strip())):
...
def extract_mentioned_relations_from_sexpr(sexpr:str)->List[str]:
    ...
    for t in toks:
        if (re_data.match("[a-zA-Z_]*\.[a-zA-Z_]*\.[a-zA-z_]*", t.strip())
            or re_data.match("[a-zA-Z_]*\.[a-zA-Z_]*\.[a-zA-Z_]*\.[a-zA-Z_]*", t.strip())):
